...
export const getTweetActionButtons = (
  article: HTMLElement
): { replyBtn: HTMLElement | null; likeBtn: HTMLElement | null } => {
  const PATH = {
  ...
  const actionGroup = Array.from(article.querySelectorAll('[role="group"]')).find(
    (g): g is HTMLElement =>
      ...
      g.querySelectorAll('button, a[role="button"]').length >= 3 &&
      !!g.querySelector("svg path")
  );
  ...
  let replyBtn: HTMLElement | null =
    buttons.find((b) => b.matches('[data-testid="reply"]')) || null;
